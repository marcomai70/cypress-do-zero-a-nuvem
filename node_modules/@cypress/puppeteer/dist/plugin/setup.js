    ...
    try {
        options.on('after:browser:launch', (browser, options) => {
            if (Number(browser.majorVersion) >= 137 && browser.name === 'chrome' && browser.isHeaded) {
                ...
                throw (0, util_1.pluginError)('@cypress/puppeteer does not work in Google Chrome v137 and higher in cypress open mode (or headed run mode). If you need to use @cypress/puppeteer in headed mode, please use Electron, Chrome for Testing, Chromium, or another Chrome variant that supports loading extensions.');
            }
